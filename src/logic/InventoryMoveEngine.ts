...
function resolveQuickSlots(item: Item): ItemSlot[] {
  ...
  const consumableDef = CONSUMABLES.find((entry) => entry.id === item.defId)

  if (THROWABLE_IDS.has(item.defId) || (consumableDef && THROWABLE_CONSUMABLE_TYPES.has(consumableDef.type))) {
