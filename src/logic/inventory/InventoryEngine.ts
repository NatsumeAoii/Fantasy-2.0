...
export const InventoryEngine = {
    ...
    getMaterial(group: keyof typeof MATERIAL_GROUPS, tier: number): MaterialDef {
        ...
        const materials = (MATERIAL_GROUPS as Record<string, MaterialDef[] | undefined>)[group];
        if (!materials || materials.length === 0) {
            // Only flatten as a last resort — this is expensive and should rarely trigger
            const fallbackMaterials = Object.values(MATERIAL_GROUPS).flat();
            if (fallbackMaterials.length === 0) return FALLBACK_MATERIAL;
            const valid = fallbackMaterials.filter(m => m.tier === effectiveTier);
            return getRandomElement(valid) || fallbackMaterials[0] || FALLBACK_MATERIAL;
        }
    ...
    generateContainer(containerDef: {
        ...
        const mechanicMetadata = applyItemMechanicModifier(getItemDefinitionMetadata({
            ...containerDef,
            capacity,
            extraSlots: containerDef.extraSlots + slotsMod,
        }), mechanicTier);
    ...
    calculateItemPrice(basePrice: number, rarity: ItemRarity): number {
        const multiplier = RARITY_PRICE_MULTIPLIERS[rarity] || 1.0;
    ...
    generateLoadout(role: string, level: number): Inventory {
        ...
        const bodyArmors = getArmorsBySlotAndType('BODY', preferredArmor);
        const bodyArmor = bodyArmors.length > 0 ? bodyArmors[0] : undefined;
        ...
        for (const slot of coreSlots) {
            const candidates = getArmorsBySlotAndType(slot, preferredArmor);
        ...
        if (inv.equipment['HANDS'] && shouldSpawn(CHANCE.HANDS_2, guarantee.accessories)) {
            const handArmor = getArmorsBySlotAndType('HANDS', preferredArmor);
        ...
        if (shouldSpawn(CHANCE.BACK_SLOT, guarantee.accessories)) {
            const backpacks = getContainersBySlot('BACK');
        ...
        if (shouldSpawn(CHANCE.SHOULDER, guarantee.accessories)) {
            const cloaks = getShoulderArmors();
            if (cloaks.length > 0) {
        ...
        const validWeapons = getWeaponsForRole(role);
        ...
        if (mainObj) {
            ...
            if (mainObj.handedness === '1H' && shouldSpawn(CHANCE.OFF_HAND, guarantee.quickslots)) {
                ...
                if (isDualWielder) {
                    if (DUAL_WIELD_OFFHANDS.length > 0) {
                        const offhand = getRandomElement(DUAL_WIELD_OFFHANDS);
                        if (offhand) {
                    ...
                    if (guarantee.quickslots || getRandom() < CHANCE.CASTER_OFFHAND) {
                        if (DAGGER_WEAPON) {
                            const daggerItem = this.generateItem(DAGGER_WEAPON, level, 'WEAPON', 'OFF_HAND');
                            daggerItem.tags = [DAGGER_WEAPON.handedness];
                    ...
                    if (guarantee.quickslots || getRandom() < CHANCE.GENERIC_OFFHAND) {
                        const offhandOptions = ONE_HANDED_WEAPONS.filter(w => w.id !== mainObj.id);
        ...
        // --- Jewelry & Belt ---
        const rings = getAccessoriesBySlot('RING');
        ...
        const neckAccessories = getAccessoriesBySlot('NECK');
        ...
        const earAccessories = getAccessoriesBySlot('EAR');
        ...
        const waistAccessories = getAccessoriesBySlot('WAIST');
        ...
        if (shouldSpawn(CHANCE.BELT, guarantee.accessories)) {
            const pouches = getContainersBySlot('BELT');
            if (pouches.length > 0) {
        ...
        // Filter potions by tier based on level — uses pre-indexed lookup
        const availableTiers = this.getPotionTiersForLevel(level);
        const filteredConsumables = getFilteredConsumables(availableTiers);
